				...
				if(length(Team))
					...
					for(var/mob/U in Team)
						...
						switch(zMissionClass)
							if("D")
								U.Rank_D++
								var/lppool = (2000 + 1.05*2000*(length(Team)-1))*lp_mult
								var/lpgain = round((lppool/length(Team))/2 + (rankgrade/totalclass)*lppool/2)
								U.body+=lpgain
								U<<"You gained [lpgain] Level Points!"
								/*U.rankkills+=1
								U<<"Gained 2 rank kills!"*/
							if("C")
								U.Rank_C++
								var/lppool = (3000 + 1.05*3000*(length(Team)-1))*lp_mult
								var/lpgain = round((lppool/length(Team))/2 + (rankgrade/totalclass)*lppool/2)
								U.body+=lpgain
								U<<"You gained [lpgain] Level Points!"
								/*U.rankkills+=2
							...
							if("B")
								U.Rank_B++
								var/lppool = (4500 + 1.05*4500*(length(Team)-1))*lp_mult
								var/lpgain = round((lppool/length(Team))/2 + (rankgrade/totalclass)*lppool/2)
								U.body+=lpgain
								U<<"You gained [lpgain] Level Points!"
								/*U.rankkills+=3
								U<<"Gained 3 rank kills!"*/
							if("A")
								U.Rank_A++
								...
								U<<"You gained [lpgain] Level Points!"
								/*U.rankkills+=4
								U<<"Gained 4 rank kills!"*/
							if("S")
								U.Rank_S++
								var/lppool = (15000 + 1.05*15000*(length(Team)-1))*lp_mult
								var/lpgain = round((lppool/length(Team))/2 + (rankgrade/totalclass)*lppool/2)
								U.body+=lpgain
								U<<"You gained [lpgain] Level Points!"
								/*U.rankkills+=5
								U<<"Gained 5 rank kills!"*/
					...
					switch(usr.MissionClass)
						if("D")
							usr.Rank_D++
							usr.body+=2000*lp_mult
							usr<<"You gained [2000*lp_mult] Level Points!"
							/*usr.rankkills+=1
							usr<<"Gained 1 rank kills!"*/
						if("C")
							usr.Rank_C++
							usr.body+=3000*lp_mult
							usr<<"You gained [3000*lp_mult] Level Points!"
							/*usr.rankkills+=2
							usr<<"Gained 2 rank kills!"*/
						if("B")
							usr.Rank_B++
							usr.body+=4500*lp_mult
							usr<<"You gained [4500*lp_mult] Level Points!"
							/*usr.rankkills+=3
							usr<<"Gained 3 rank kills!"*/
						if("A")
							usr.Rank_A++
							usr.body+=7000*lp_mult
							usr<<"You gained [7000*lp_mult] Level Points!"
							/*usr.rankkills+=4
							usr<<"Gained 4 rank kills!"*/
						if("S")
							usr.Rank_S++
							usr.body+=15000*lp_mult
							usr<<"You gained [15000*lp_mult] Level Points!"
							/*usr.rankkills+=5
							usr<<"Gained 5 rank kills!"*/
	...
	proc
		GetMission(rank)
			...
			if(rank=="S")//Very rare, programmed events, sometimes yeild special items(techniques even)
				...
				usr.MissionClass="S"
				alert(usr,"Here is your S-Rank Mission Scroll, This is an urgent mission and you only have 30 minutes to do it!(You fail if you sleep or die)")
		...
		GetMission2(indec,list/targetl)
			...
			switch(indec)
				if(1)
					...
					usr.pay=rand(150, 200) + rand(150, 200)
				if(2)
				...
				//	usr.MissionClass="C"
					...
					usr.pay=rand(220, 340) + rand(220, 340)
				if(3)
			...
			//		usr.MissionClass="B"
					...
					usr.TargetLocation=X.locationdisc
					usr.pay=rand(350, 400) + rand(350, 400)
				if(4)
					...
					usr.pay=rand(400, 500) + rand(400, 500)
				if(5)
