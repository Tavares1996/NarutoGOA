...
obj/clay_kawa
	...
	proc/Explode()
		if(src.icon)
			...
			var/power=rand(700,(1500+400*conmult))
...
obj
	...
	gui
		...
		MouseDrop(obj/gui/over_object,src_location,over_location)
			if(src==over_object)
				return
			if(/*istype(over_object,/obj/gui/placeholder) ||*/istype(over_object,/skillcard)||istype(over_object,/obj/items))
				if(!usr)
					return
				else
					var/spot
					...
					if(istype(over_object,/skillcard)) usr.client.screen-=over_object
					...
					switch(S)
						...
						//	X.deletable=1
							...
							for(var/obj/gui/o in usr.client.screen)
								if(o.posit==1)
									usr.client.screen-=o
							for(var/skillcard/o in usr.client.screen)
								if(o.posit==1)
									usr.client.screen-=o
