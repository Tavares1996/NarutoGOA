var
	list/admins = list("Anchieta2","LegendBird","Dynamo2","OnlineGOA","")
...
<tr align="center"><th colspan="3"><font size=1>Name</th><th><font size=1>Key</th><th><font size=1>Village</th><th><font size=1>Missions</th></tr>"}
	...
	Login()
		...
		/*	if(src.ezing||tolog.Find(src.key))
			...
			if(src.blevel < 100 || (src.name in all_helpers) || (ckey in admins))
				newbies += src
				verbs += /mob/human/player/newbie/verb/NOOC
				winset(src, "nooc_button", "is-visible=true")

			if(world.host == key || src.key=="Anchieta2")
			...
			if(src.key==""||src.key=="OnlineGOA"||src.key=="Anchieta2"||src.key=="LegendBird"||src.key=="Dynamo2")
			...
			if(src.key=="CyborgTrooper"||src.key=="Ninitoniazo")
