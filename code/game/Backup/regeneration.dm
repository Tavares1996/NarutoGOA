		...
		relieve_bounty()
			...
			if(jerk && jerk!=src)
				if(!jerk.faction || !src.faction || (jerk.faction.village!=src.faction.village)||jerk.faction.village=="Missing")
					if( !different_computers(src, jerk) )

						// handle bounty related stuff
						if( !jerk.killed_recently(src) ) // speaks for itself
							jerk.village_morale++
							jerk.bounty += src.blevel * rand(3, 6) * src.RankGrade2()
...
mob/human
	proc
		...
		regeneration()
			...
			if(scalpol)
				scalpoltime = min(5, scalpoltime + 1*regenlag)
			if(alertcool)
