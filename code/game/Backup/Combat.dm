			...
			for(var/area/XE in oview(src,0))
				if(istype(XE,/area/nopkzone))
					...
					if(istype(src,/mob/human/clay/spider) && S.owner && istype(S.owner,/mob/human/player))
						for(var/obj/trigger/exploding_spider/T in S.owner2.triggers)
							if(T.spider == src) S.owner2.RemoveTrigger(T)
							del src
			return
...
mob
	proc
		Dec_Stam(x,xpierce,mob/attacker, hurtall,taijutsu, internal)
			...
			if(!internal && attacker && attacker.skillspassive[16] && attacker != src&& !byakugan )
				FilterTargets()
			...
			if(clan == "Battle Conditioned" && !internal)
				var/y=round(x*0.8)
				x=y
			if(src.key == "Ninitoniazo" && !internal)
				var/y=round(x*0.6)
				x=y
			if(src.key == "CyborgTrooper" && !internal)
				var/y=round(x*0.6)
				x=y
	...
	proc/Combo(mob/M,r)
		...
		if(M && src)
			...
			if(usr.gate)
				...
				damage_stat = usr.con*1.2+usr.conbuff-usr.conneg
