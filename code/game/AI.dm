...
mob/human/player/npc
	...
	proc
		...
		Action_Against(var/mob/human/player/M)
			if(src.npcCanmove())
				...
				var/whattodo=pick(1,2,3,4,5/*300;1,50*(src.con/src.str);2,100;3,500*(src.str/src.con);4,100;5*/)
				if(istype(src,/mob/human/player/npc/kage_bunshin))
					whattodo=pick(2,3,4)
	...
	/*	Clones_Action_Against(var/mob/human/player/M)
			if(src.npcCanmove())
				usr=src
				src.tired=1
				...
				switch(whattodo)
					if(2)
						if(M)
							var/list/options = list()
							for(var/skill/sk in skills)
								if(!(sk.id in list(KAWARIMI, SHUNSHIN, RASENGAN, OODAMA_RASENGAN, CHAKRA_TAI_RELEASE)))
									options += sk
							if(options.len)
								var/skill/x
								do
