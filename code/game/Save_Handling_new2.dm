...
/*client/var/get_save == "get_save"
client/var/save_file = "Saves/[src.ckey].sav"

world/proc/Topic(href, addr)
	var/T[] = params2list(href)

	if("get_save")
		. = save_file(T["char"], T["key"])
		...
		ChooseCharacter()
			if(src.client/*&&src.client.chars*/)
				...
				if(length(menu))
					...
					var/list/Char=dd_text2List(SendInterserverMessage("savefile", list("char" = pickd, "key" = ckey)),"$")
	...
	proc
		DeleteCharacter()
			...
			if(src.client)
				...
				if(!length(menu))
					return
				menu += "Cancel"
				var/pickd = input2(src, "Delete a Character","Delete",menu)
				if(pickd=="Cancel")
					return
				//save_num = pickd
				fdel("Saves/[src.ckey]/[src.ckey][pickd].sav")
				src<<"Char Deleted ..."
			/*	var/list/delete = new()
				alert("WARNING: Your about to select a character to delete.")
				...
				if(length(delete))
					delete += "Cancel"
					var/pkcd = input("Delete character", "DELETE",delete)
					if(pkcd=="Cancel")
						return
					if(pkcd==delsav1)
						fdel(delsav1)
		...
		/*		var/CancelCharacterDeletion = "Cancel"
				...
				if (result && result!="Cancel")
					characters-=result
					F=new("Saves/[src.ckey].sav")
					F["Characters"]<<characters
					if(fexists("Saves/[ckey(result)].sav"))*
						fdel("Saves/[ckey(result)].sav")*/
...
mob
	proc
		...
		NewCharacter()
			...
			if (HasFreeSaves() == 0)
				alert(src,"You have [max_saves] characters, please delete one before creating a new character.")
				return
