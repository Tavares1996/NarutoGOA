...
proc
	Save_Faction_Names()
		if(length(Faction_Names))
			var/savefile/F = new("Saves/GameSaves/Faction Names.sav")
			F["Names"] << Faction_Names
	Load_Faction_Names()
		if(fexists("Saves/GameSaves/Faction Names.sav"))
			var/savefile/F = new("Saves/GameSaves/Faction Names.sav")
			F["Names"] >> Faction_Names
...
var
	list
		faction_mouse = list(
			...
			"Iwa" = 'icons/mouse_icons/rockmouse.dmi',
			"Crimzon" = 'crimzonmouse.dmi',
			"Kumo" = 'cloudmouse.dmi',
			"Star" = 'star-mouse.dmi',
		)
		faction_chat = list(
			...
			"Zelko-Blitzkreig" = 'faction_icons/zelko-blitzkreig-chat.dmi',
			"Iwa" = 'pngs/Rock.png',
			"Deva-Crimzon" = 'pngs/crimzonchat.png',
			"Star" = 'faction_icons/star-chat.dmi',
		)
...
faction
	...
	proc
		...
		Save_Faction()
			if(src.name == "Konohagakure"||src.name == "Kirigakure"||src.name == "Sunagakure"||src.name == "Missing")
				return
			if(src)
				var/savefile/F = new("Saves/GameSaves/Factions/[src.name].sav")
				F["village"] << "[src.village]"
				F["leader"] << "[src.leader]"
				F["mouse_icon"] << "[src.mouse_icon]"
				F["chat_icon"] << "[src.chat_icon]"
				F["chuunin_item"] << "[src.chuunin_item]"
				F["member_limit"] << "[src.member_limit]"
...
proc
	initialize_basic_factions()
		...
		faction_info = list("name"="Konohagakure", "village"="Konoha", "leader"="Nara Atticus", "mouse_icon"="Konoha", "chat_icon"="Konoha", "chuunin_item"="224", "member_limit"=0)
		...
		faction_info = list("name"="Kirigakure", "village"="Kiri", "leader"="Uchiha Reyn", "mouse_icon"="Kiri", "chat_icon"="Kiri", "chuunin_item"="226", "member_limit"=0)
		...
		faction_info = list("name"="Sunagakure", "village"="Suna", "leader"="Naruke", "mouse_icon"="Suna", "chat_icon"="Suna", "chuunin_item"="225", "member_limit"=0)
		...
		faction_info = list("name"="Missing", "village"="Missing", "leader"=null, "mouse_icon"=null, "chat_icon"="Missing", "member_limit"=0)
		...
		faction_info = list("name"="Rain", "village"="Missing", "leader_name"="", "mouse_icon"="Ourico-Ame", "chat_icon"="Ourico-Ame", "chuunin_item"="10000", "member_limit"=10)
		rain_faction = new /faction(faction_info["name"], faction_info["village"], faction_info["leader"], faction_info["mouse_icon"], faction_info["chat_icon"], faction_info["chuunin_item"], faction_info["member_limit"], 0)

		faction_info = list("name"="Sound", "village"="Sound", "leader"="Hatake Ragna", "mouse_icon"="Mange-Sound", "chat_icon"="Mange-Sound", "chuunin_item"="10001", "member_limit"=10)
		sound_faction = new /faction(faction_info["name"], faction_info["village"], faction_info["leader"], faction_info["mouse_icon"], faction_info["chat_icon"], faction_info["chuunin_item"], faction_info["member_limit"], 0)

		faction_info = list("name"="Iwagakure", "village"="Missing", "leader_name"="", "mouse_icon"="Iwa", "chat_icon"="Iwa", "chuunin_item"="999999999", "member_limit"=10)
		iwa_faction = new /faction(faction_info["name"], faction_info["village"], faction_info["leader"], faction_info["mouse_icon"], faction_info["chat_icon"], faction_info["chuunin_item"], faction_info["member_limit"], 0)

		faction_info = list("name"="Crimzon Noire", "village"="Missing", "leader_name"="", "mouse_icon"="Crimzon", "chat_icon"="Deva-Crimzon", "chuunin_item"="123456789", "member_limit"=10)
		crimzon_faction = new /faction(faction_info["name"], faction_info["village"], faction_info["leader"], faction_info["mouse_icon"], faction_info["chat_icon"], faction_info["chuunin_item"], faction_info["member_limit"], 0)

		faction_info = list("name"="Zelko Blitzkreig", "village"="Missing", "leader_name"="", "mouse_icon"="Zelko-Blitzkreig", "chat_icon"="Zelko-Blitzkreig", "chuunin_item"="263", "member_limit"=10)
		blitz_faction = new /faction(faction_info["name"], faction_info["village"], faction_info["leader"], faction_info["mouse_icon"], faction_info["chat_icon"], faction_info["chuunin_item"], faction_info["member_limit"], 0)

		faction_info = list("name"="Star", "village"="Missing", "leader_name"="", "mouse_icon"="Star", "chat_icon"="Star", "chuunin_item"="", "member_limit"=10)
		star_faction = new /faction(faction_info["name"], faction_info["village"], faction_info["leader"], faction_info["mouse_icon"], faction_info["chat_icon"], faction_info["chuunin_item"], faction_info["member_limit"], 0)
		...
		rain_faction.tag = "faction__[rain_faction.name]"
		sound_faction.tag = "faction__[sound_faction.name]"
		missing_faction.tag = "faction__[missing_faction.name]"
		iwa_faction.tag = "faction__[iwa_faction.name]"
		crimzon_faction.tag = "faction__[crimzon_faction.name]"
		blitz_faction.tag = "faction__[blitz_faction.name]"
		star_faction.tag = "faction__[star_faction.name]"
	...
	load_faction(faction_name)
		...
		switch(faction_name)
			...
			if("Zelko Blitzkreig")
				faction = blitz_faction
			if("Rain")
				faction = rain_faction
			if("Sound")
				faction = sound_faction
			if("Iwagakure")
				faction = iwa_faction
			if("Crimzon")
				faction = crimzon_faction
			if("Star")
				faction = star_faction
		...
		if(!faction)
			if(faction_name in Faction_Names)
				if(fexists("Saves/GameSaves/Factions/[faction_name].sav"))
					var/savefile/faction_info = new("Saves/GameSaves/Factions/[faction_name].sav")
					faction = new /faction(faction_name, faction_info["village"], faction_info["leader"], faction_info["mouse_icon"], faction_info["chat_icon"], faction_info["chuunin_item"], faction_info["member_limit"], 0)
					faction.tag = "faction__[faction.name]"
					faction.Save_Faction()
				else
	...
	proc
		Refresh_Faction_Verbs()
			...
			if(faction)
				...
				if(faction in list(leaf_faction, mist_faction, sand_faction, sound_faction))
					if(faction.leader == realname)
		...
		verb
			Faction_Say(msg as text)
				...
				if(mute||tempmute)
					...
					talkcool=20
					talktimes+=1
					if(talktimes>=5 && rank=="Academy Student")
						src<<"Sorry new players cannot talk that fast, take a breather before each message."
						return
		...
		default_non_missing
			...
			/*	Set_Info_Card_Comment(mob/M in world)
					...
					if(comment)
						...
						if(M.faction.village && src.faction.village == "Konohagakure" || M.faction.village && src.faction.village == "Kirigakure" || M.faction.village && src.faction.village == "Sunagakure")
							success = 1
						if(success==1)
							comment = "comment"
...
*/
				...
				Send_to_Arena()
					...
					if(x && x.client && !x.incombo && !x.inarena && !isCombatFlag(60))
						if(x.shopping)
							x.shopping=0
							x.canmove=1
							x.see_invisible=0
						...
						usr << "Unable to add [x.name] to the arena due to recent combat. Please try again in one minute."
				Start_Fight()
					set category="Faction Leader"
				...
				Invite_to_Village()
					...
					for(var/mob/human/X in world)
						if(X.client && X.faction && X.faction.name == "Missing" && X.faction.village =="Missing" && X.faction.name == "Sound" && X.faction.village =="Sound") li+=X
					var/list/l2=li.Copy()
