		...
		New(loc,mob/cause)
			...
			spawn(1)
				...
				for(var/mob/human/player/X in oview(0,src))
					if(!X.icon_state)
						flick("hurt",X)
					X.Dec_Stam(rand(2500,5000) + 2000*conbuff, 0, cause)
					X.Hostile(cause)
					X.Wound(10, 0, cause)
					spawn()Blood2(X)
					X.move_stun+=30
				...
				del(src)
proc
	AmatCircle(bx,by,bz,finrad,mob/cause)
		var/rad=0
		var/rad2=0
		var/nx=bx
		var/ny=by
		...
		while(rad<finrad)
			...
			spawn()
				var/mx2=nx2
				if(rad2==1)
					listx+=new/obj/Amaterasu(locate(bx,ny+rad2,bz),cause)
					listx+=new/obj/Amaterasu(locate(bx,ny-rad2,bz),cause)
				else
					listx+=new/obj/Amaterasu(locate(bx,ny+rad2-1,bz),cause)
					listx+=new/obj/Amaterasu(locate(bx,ny-rad2+1,bz),cause)
				while(mx2>bx)


					listx+=new/obj/Amaterasu(locate(mx2,ny-diff2,bz),cause)
					listx+=new/obj/Amaterasu(locate(mx2,ny+diff2,bz),cause)
					mx2-=2
			...
			spawn()
				...
				while(mx>bx)
					if(diff)
						listx+=new/obj/Amaterasu(locate(mx,ny+diff,bz),cause)
						listx+=new/obj/Amaterasu(locate(mx,ny-diff,bz),cause)
			...
			spawn()
				var/mx2=nx4
				if(rad2==1)
					listx+=new/obj/Amaterasu(locate(bx+rad2,ny,bz),cause)
					listx+=new/obj/Amaterasu(locate(bx-rad2,ny,bz),cause)
				else
					listx+=new/obj/Amaterasu(locate(bx+rad2-1,ny,bz),cause)
					listx+=new/obj/Amaterasu(locate(bx-rad2+1,ny,bz),cause)
				while(mx2<bx)


					listx+=new/obj/Amaterasu(locate(mx2,ny+diff4,bz),cause)
					listx+=new/obj/Amaterasu(locate(mx2,ny-diff4,bz),cause)
					mx2+=2
			...
			spawn()
				...
				while(mx<bx)
					if(diff3)
						listx+=new/obj/Amaterasu(locate(mx,ny+diff3,bz),cause)
						listx+=new/obj/Amaterasu(locate(mx,ny-diff3,bz),cause)
...
mob/human
	amaterasu
		...
		var/tired=0
		density=1
		icon='icons/ama-flames.dmi'
		icon_state=""

		New()
			src.underlays+=image('icons/ama-up.dmi',pixel_y=-10)
			src.underlays+=image('icons/ama-down.dmi',pixel_y=10)
			src.underlays+=image('icons/ama-right.dmi',pixel_x=-10)
			src.underlays+=image('icons/ama-left.dmi',pixel_x=10)
			spawn()
				src.nopkloop()
		proc
			P_Attack(mob/human/etarget,mob/human/owner)
				...
				for(var/mob/human/x in oview(7,src))
					if(x==etarget)
						...
						if(get_dist(src,x)<=1)hit=1
							...
							sleep(1)
						if(hit)
							src.overlays+=image('icons/amaterasu.dmi',icon_state="sand-spikes")
							spawn(9)
								src.overlays-=image('icons/amaterasu.dmi',icon_state="sand-spikes")
								src.density=0
								walk_to(src,owner,0,1)
								...
								walk(src,0)
							etarget.Dec_Stam(src.con*rand(50,150)/100)
							etarget.Wound(src.int*rand(1,2)/100)
							if(!etarget.icon_state)
								flick("hurt",etarget)
							etarget.Hostile(owner)
...
obj/gatesaura
	...
	tr
		...
		pixel_y=32
obj/firedr
	icon='icons/greatdragonflames.dmi'
	gdr1
	...
	gdr8
		...
		pixel_y=32
obj/cswing1
	icon='icons/SasukeLeftWing.dmi'
	lw
		icon_state="lw"
		pixel_x=-16
obj/cswing2
	icon='icons/SasukeRightWing.dmi'
	rw
		icon_state="rw"
		pixel_x=16
obj/oodamaexplosion
...
/*proc
	dry_proj(dx,dy,dz,eicon,estate,mob/human/u,dist,epower,emag)
		if(emag>=1)
			if(u.dir==NORTH)
				Dry_cap(u.x,u.y,u.z,SOUTH,emag,1200)
			if(u.dir==SOUTH)
				Dry_cap(u.x,u.y,u.z,NORTH,emag,1200)
			if(u.dir==WEST)
				Dry_cap(u.x,u.y,u.z,EAST,emag,1200)
			if(u.dir==EAST)
				Dry_cap(u.x,u.y,u.z,WEST,emag,1200)*/
...
/*		if(M.dir==NORTH)
		...
		if(M.dir==WEST)
			M.x--
		if(emag>=1)
			Dry_cap(M.x,M.y,M.z,M.dir,emag,1200)
		sleep(1)
		del(M)
proc/Dry_cap(dx,dy,dz,xdir,mag,xdur)
	spawn()
		if(mag==1)
			var/obj/Sand_sides/sss1

			if(xdir==NORTH)
				sss1=new/obj/Sand_sides/sss1(locate(dx,dy,dz))
			if(xdir==SOUTH)
				sss1=new/obj/Sand_sides/sss1(locate(dx,dy,dz))
			if(xdir==WEST)
				sss1=new/obj/Sand_sides/sss1(locate(dx,dy,dz))
...
obj
	trail
		...
		Ougi_Fire_Dragon_Trail
			layer=OBJ_LAYER
			icon='icons/Ougi_Fire_Dragon_Trail.dmi'
		falsedarknesstrail
			layer = OBJ_LAYER
	...
	trailmaker
		...
		Ougi_Fire_Dragon
			density=1
			layer=MOB_LAYER+2
			icon='icons/Ougi_Fire_Dragon.dmi'
			...
			Move()
				...
				spawn()
					if(d)
						var/obj/O = new(old_loc)
						O.dir = src.dir
						var/obj/m=new/obj/trail/Ougi_Fire_Dragon_Trail(O)
						m.dir=O.dir
						m.icon_state="patch"
						var/obj/n=new/obj/trail/Ougi_Fire_Dragon_Trail(O)
						n.dir=O.dir
						n.icon_state="patch"
						if(O.dir==NORTHEAST)
							src.pixel_y=16
						...
						O.underlays+=m
						spawn(1)O.underlays+=n
						O.icon = 'icons/Ougi_Fire_Dragon_Trail.dmi'
						src.trails += O
				return d
...
proc
	explosion_spread(power,dx,dy,dz,mob/human/user,dontknock)

		var/conmult = user.ControlDamageMultiplier()

		spawn()explosion(1000+500*conmult,dx,dy,dz,user,0,6)
...
obj
	shu_wind
		New()
			src.overlays+=image('icons/ShukakuWind.dmi',icon_state = "wind_r",pixel_x=16)
			src.overlays+=image('icons/ShukakuWind.dmi',icon_state = "wind_n",pixel_y=16)
			src.overlays+=image('icons/ShukakuWind.dmi',icon_state = "wind_tr",pixel_x=16,pixel_y=16)
			src.overlays+=image('icons/ShukakuWind.dmi',icon_state = "ur",pixel_x=16,pixel_y=16)
