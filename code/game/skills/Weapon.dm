skill
		...
		twin_rising_dragons
			...
			Use(mob/user)
				...
				while(ammo>0)
					...
					spawn() advancedprojectile_angle('icons/twin_proj.dmi', "[pick(1,2,3,4)]", user, speed, angle, distance=7, damage=1500, wounds=5, radius=16)
	...
	/*	lightning_blade_creation
			id = LIGHTNING_BLADE_CREATION
			name = "Summoning: Lightning Blade Creation"
			icon_state = "Lightning Blade Creation"
			...
			Use(mob/user)
				viewers(user) << output("[user]: Summoning: Lightning Blade Creation", "combat_output")
				user.stunned=100
				flick("Throw1",user)
				spawn(1)Poof(user.x,user.y,user.z)
				user.supplies+=10
				user.stunned=0
				if(user.supplies >= 500)
					user.supplies = 500*/