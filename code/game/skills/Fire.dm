	...
	fire
		...
		grand_fireball
			...
			Use(mob/human/user)
				...
				if(dir == NORTH)
					spawn() AOE(user.x, user.y + 3, user.z, 2, (100 + conmult * 60), 50, user, 3, 2)
					spawn() Fire(user.x, user.y + 3, user.z, 2, 50)

				if(dir == SOUTH)
					spawn() AOE(user.x, user.y - 3, user.z, 2, (100 + conmult * 40), 50, user, 3, 2)
					spawn() Fire(user.x, user.y- 3, user.z, 2, 50)

				if(dir == EAST)
					spawn() AOE(user.x + 3, user.y, user.z, 2, (100 + conmult * 40), 50, user, 3, 2)
				...
				spawn()
					...
					if(!etarget)
						...
						if(etarget)
							...
							var/ey=etarget.y
							var/ez=etarget.z
							spawn()AOE(ex,ey,ez,1,(100 +30*conmult),20,user,3,1,1)
		...
		burning_ash
			...
			Use(mob/human/user)
				...
				//AOE(x,y,z,radius,stamdamage,duration)
				if(dir==NORTH)
					spawn()AOE(user.x,user.y+5,user.z,4,(85+50*conmult),90,user,2,1,3)
					spawn()Ash(user.x,user.y+5,user.z,100)
				if(dir==SOUTH)
					spawn()AOE(user.x,user.y-5,user.z,4,(85+50*conmult),90,user,2,1,3)
					spawn()Ash(user.x,user.y-5,user.z,100)
				if(dir==EAST)
					spawn()AOE(user.x+5,user.y,user.z,4,(85+50*conmult),90,user,2,1,3)
