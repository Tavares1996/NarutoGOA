...
mob/human/player/npc/creep
	...
	overshirt
	armor=/obj/item_appearence/plate/body/brown
...
mob/human/player/npc
	proc
		npcregeneration2()
			...
			if(src.alertcool)
				src.alertcool = max(0, src.alertcool - 1 * npcdelay2)
			//	if(src.alertcool<0)
			//		src.alertcool=0
			...
			/*	else if(istype(A,/area/dojo))
				...
				if(src.curstamina<=0)
					src.Wound(0,rand(32,37),src,"KO","Internal")
				...
				if(curwound<maxwound&&!mane&&!maned &&!waterlogged)
					if(curchakra<chakra)
						if(chakraregen > (chakra-curchakra))
							...
							curchakra +=chakraregen*npcdelay2
						...
						if(curstamina<stamina)
							if((staminaregen) > (stamina-curstamina))
								...
								curstamina+=staminaregen*npcdelay2

				if(!dead)
					spawn()npcregeneration2()
