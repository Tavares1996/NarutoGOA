			...
			Watch_Fight_Chuunin()
				...
				if(team_deathmatch)
					for(var/mob/human/player/m in world)
						if(m in team_deathmatch.white_team || m in team_deathmatch.orig_white_team) li += m
						if(m in team_deathmatch.black_team || m in team_deathmatch.orig_black_team) li += m
...
turf/memberwarp
	Enter(mob/human/O)
		if(istype(O) && O.client)
			if(!O.client.IsByondMember()&&!O.key in admins)
